            ...
            $.each(data, function (i, item) {
                if (item[options.parentColumn] == parentNode[options.id]) {
                    ...
                    $.each(options.columns, function (index, column) {
                        var td = $('<td></td>');
                        //column 传入参数内容,title,field,width等等
                        //index 行数
                        //item 当前内容
                        if(options.columns[index].formatter){
                        	//传入行数，field内容，整行内容
                        	td.html(options.columns[index].formatter(index,item[column.field],item));
                        }else{
        ...
        if (options.url) {
            $.ajax({
                ...
                success: function (data, textStatus, jqXHR) {
                    //debugger;
                    //构造表头
                    var thr = $('<tr class="dark"></tr>');
                    $.each(options.columns, function (i, item) {
                    	//增加表头单元占比百分比
                        var th = $('<th style="padding:10px;width:'+item.width+'px;"></th>');
                        th.text(item.title);
                        thr.append(th);
                    });
