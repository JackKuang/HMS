        ...
        ,function(){
          return options.check ? (
            '<i class="layui-icon layui-tree-check'+ (
            	item.selected === 1 ? ' lee-onChecked':''
            )
            +'" value="' + item.id + '">'+ (
              options.check === 'checkbox' ? (item.selected === 1 ? icon.checkbox[1] : icon.checkbox[0]) : (
                options.check === 'radio' ?  (item.selected === 1 ? icon.radio[1] : icon.radio[0]) : ''
              )
            ) +'</i>'
          ) : '';
        }()
  ...
  Tree.prototype.click = function(elem, item){
    ...
    elem.children('i.layui-tree-check').on('click', function(e){
        if($(this).hasClass("lee-onChecked")){//已勾选，需要取消勾选
            ...
            //子节点取消勾选自动取消父节点勾选
            var childLI = $(this).parent().parent().find("i.lee-onChecked");
            if(childLI.length <= 0){
                var parentCheck = $(this).parent().parent().parent().find("i.layui-tree-check");	
                parentCheck.html("&#xe626;");
                parentCheck.removeClass("lee-onChecked");
            }
        }else{
            $(this).html("&#xe627;");
            ...
            var parentCheck = $(this).parent().parent().parent().find("i.layui-tree-check:first");	
            parentCheck.html("&#xe627;");
            parentCheck.addClass("lee-onChecked");
        }
    });
  };
